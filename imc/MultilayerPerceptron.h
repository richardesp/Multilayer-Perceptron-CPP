...
// ---------------------
    ...
    private:
        ...
        // Update the network weights, from the last layer to the first one
        void weightAdjustmentOffline();
        ...
        // errorFunction=1 => Cross Entropy // errorFunction=0 => MSE
        void
        performEpoch(double *input, double *target, int errorFunction);

        // Perform an epoch: forward propagate the inputs, backpropagate the error and adjust the weights
        // input is the input vector of the pattern and target is the desired output vector of the pattern
        void performEpochOnline(double *input, double *target, int errorFunction);
    ...
    public:
        ...
        // errorFunction=1 => Cross Entropy // errorFunction=0 => MSE
        void train(Dataset *trainDataset, int errorFunction, bool onlineMode = false); // Online mode by default
        ...
        // errorFunction=1 => Cross Entropy // errorFunction=0 => MSE
        void runBackPropagation(Dataset *trainDataset, Dataset *testDataset, Dataset *validationDataset, int maxiter,
                                double *errorTrain,
                                double *errorTest, double *ccrTrain, double *ccrTest, int errorFunction,
                                std::vector<double> &trainErrorVector,
                                std::vector<double> &testErrorVector, bool verbose = false,
                                bool learningRateScheduler = false, char *learningRateSchedulerTpe = NULL,
                                bool printCrossValidation = false, bool onlineMode = false,
                                bool printCurrentCCR = false);
        ...
        double calculateNet(int layer, int neuron);

        void softmax(int layerIndex);

        void sigmoid(int layerIndex);

        double getMSE(const double *target);

        double getCrossEntropy(const double *target);

        double getMSE(double *output, double *target, int nOfOutputs);
    };
